...
%             for i = 1:dxf.ne
%                 disp(dxf.entities(i));
%                 disp(dxf.entities(i).poly);
%             end
            for i = 1:dxf.ne
                if isempty(dxf.entities(i).name)
                    continue
                end
                % get colors
...
function scaleFactor = getScaleFactor(dxfUnit, targetUnit)
    % 定义单位转换的比例因子
    ...
    % 获取单位对应的比例因子
    fromFactor = unitConversion.(dxfUnit);
    toFactor = unitConversion.(targetUnit);
    
    % 计算比例因子
    scaleFactor = fromFactor / toFactor;
end
...
%  42:  End parameter (this value is 2pi for a full ellipse)
    ...
    for i = 1:length(EntPos)-2
        ...
        if strcmp(AllValues{EntPos(i+1)},'VERTEX')||strcmp(AllValues{EntPos(i+1)},'SEQEND')
            continue
        end
        entities(k).name        = AllValues{EntPos(i+1)};
        entities(k).layer       = eStrings{eCodes==8};
        try
            entities(k).linetype    = eStrings{eCodes==6};
        catch
            entities(k).linetype    = '';
        end
        ...
        if isempty(color)
            ...
            entities(k).color = color;
        end
        
        switch upper(entities(k).name)
            case 'HATCH'
                % just store it for coloring previous entity.
                k=k+1;
            case 'SPLINE'
                entities(k).spline.degree     = get_values(71,eCodes,eValues);
                entities(k).spline.no_knots   = get_values(72,eCodes,eValues);
                entities(k).spline.no_control = get_values(73,eCodes,eValues);
                entities(k).spline.knot_value = get_values(40,eCodes,eValues);
                entities(k).spline.X          = get_values(10,eCodes,eValues);
                entities(k).spline.Y          = get_values(20,eCodes,eValues);
                k=k+1;
            case 'LINE' 
                % (Xi,Yi,Xj,Yj) start and end points
                entities(k).line = [get_values(10,eCodes,eValues),...
            ...
            case 'POLYLINE'
                %是否闭合
                entities(k).closed = 0; % 初始化为不闭合

                % 获取 POLYLINE 的 flag (bit-coded)
                flags = get_values(70, eCodes, eValues);
                % 如果 flag 存在，检查是否闭合
                if ~isempty(flags)
                    entities(k).closed = bitand(flags, 1); % 检查闭合位
                end
                
                vertices = [];
                for j = i+1:length(EntPos)-1
                    str = upper(AllValues{EntPos(j)});
                    if strcmp(str, 'VERTEX')
                        % Extract the vertex data
                        vx = get_values(10, AllCodes(EntPos(j):EntPos(j+1)-1), str2double(AllValues(EntPos(j):EntPos(j+1)-1)));
                        vy = get_values(20, AllCodes(EntPos(j):EntPos(j+1)-1), str2double(AllValues(EntPos(j):EntPos(j+1)-1)));
                        bulge_verts = get_values(42, AllCodes(EntPos(j):EntPos(j+1)-1), str2double(AllValues(EntPos(j):EntPos(j+1)-1)));
            
                        if isempty(bulge_verts)
                            bulge_verts = 0;
                        end
                        
                        if bulge_verts == 0
                            % If bulge is 0, just add the vertex
                            vertices = [vertices; [vx, vy]];
                        else
                            % Handle the bulge to create additional vertices for the arc
                            if j < length(EntPos) - 1
                                next_vx = get_values(10, AllCodes(EntPos(j+1):EntPos(j+2)-1), str2double(AllValues(EntPos(j+1):EntPos(j+2)-1)));
                                next_vy = get_values(20, AllCodes(EntPos(j+1):EntPos(j+2)-1), str2double(AllValues(EntPos(j+1):EntPos(j+2)-1)));
                                arc_points = calculate_bulge_arc([vx, vy], [next_vx, next_vy], bulge_verts);
                                vertices = [vertices; arc_points];
                            else
                                vertices = [vertices; [vx, vy]];
                            end
                        end
                    elseif strcmp(str, 'SEQEND')
                        % End of polyline
                        break;
                    end
                end
                entities(k).poly = vertices*scale;
                k=k+1;
            ...
            case 'LWPOLYLINE' 
                ...
                % closed polygon
                entities(k).closed = get_values(70,eCodes,eValues);
                ...
                entities(k).poly = verts;               
                k=k+1;
            case 'CIRCLE' 
                % (X Center,Y Center,Radius)
                entities(k).circle = [get_values(10,eCodes,eValues),...
                                      get_values(20,eCodes,eValues),...
                                      get_values(40,eCodes,eValues)];
                k=k+1;
            case 'ARC' 
                % (X Center,Y Center,Radius,Start angle,End angle)
                entities(k).arc = [get_values(10,eCodes,eValues),...
                                   get_values(20,eCodes,eValues),...
                                   get_values(40,eCodes,eValues),...
            ...
            case 'POINT' 
                % (X,Y) Position
                entities(k).point = [get_values(10,eCodes,eValues),...
                                     get_values(20,eCodes,eValues)]; %#ok<*AGROW>
                k=k+1;
            
            case 'ELLIPSE' 
                % X center ,Y center, X end, Y end, ratio, start, end
                entities(k).ellipse = [get_values(10,eCodes,eValues),...
                                       get_values(20,eCodes,eValues),...
                                       get_values(11,eCodes,eValues),...
                                       get_values(21,eCodes,eValues),...
                                       get_values(40,eCodes,eValues),...
                ...
                if entities(k).ellipse(6)==0 && entities(k).ellipse(7)>0.999*2*pi
                    entities(k).closed = 1;
                else
                    entities(k).closed = 0;
                end
